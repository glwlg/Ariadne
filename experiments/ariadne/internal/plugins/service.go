...
func NewService() *Service {
	return NewServiceWithLegacyBridge(nil)
}

func NewServiceWithLegacyBridge(legacy LegacyBridge) *Service {
...
func legacyPythonManifest(legacy LegacyBridge) registeredPlugin {
	return manifest("legacy_python", "Python 旧插件桥", "按需运行旧版 Python 插件作为过渡方案", []string{"legacy", "py"}, []string{"python_legacy_bridge"}, CommandSchema{
		Usage:    "legacy <plugin-keyword> [query]",
		Examples: []string{"legacy calc 1+1", "py timestamp now"},
		Params: []CommandParam{
			{Name: "plugin_keyword", Label: "旧插件关键词", Placeholder: "例如: calc / timestamp", Required: true},
			{Name: "query", Label: "查询", Placeholder: "传给旧插件的参数", Required: false},
		},
	}, executeLegacyPython(legacy))
}
...
func executeLegacyPython(legacy LegacyBridge) handler {
	return func(query string) []contracts.SearchResult {
		keyword, rest := splitCommand(query)
		if keyword == "" {
			return []contracts.SearchResult{messageResult("legacy-python-help", "输入旧插件关键词", "legacy <plugin-keyword> [query]", "旧插件桥只用于过渡，不作为 Ariadne 长期插件主路径。", "plugin")}
		}
		if legacy == nil {
			return []contracts.SearchResult{messageResult("legacy-python-disabled", "Python 旧插件桥未启用", keyword, "当前运行实例没有挂载 Python legacy bridge。", "error")}
		}
		results := legacy.Execute(keyword, rest)
		if len(results) == 0 {
			return []contracts.SearchResult{messageResult("legacy-python-empty", "旧插件无结果", keyword, "旧插件没有返回可展示结果。", "plugin")}
		}
		return results
	}
}
