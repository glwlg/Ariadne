...
func (a *CodexFlowAgent) AnswerFlow(ctx context.Context, job workmemory.FlowAgentJob) (workmemory.FlowAgentResult, error) {
	...
	if len(job.Evidence) == 0 {
		return workmemory.FlowAgentResult{}, errors.New("flow agent evidence 为空")
	}
	...
	return workmemory.FlowAgentResult{
		Answer:  answer,
		Mode:    "agent:codex",
		Message: "Codex agent 已基于本地证据生成回答。",
	}, nil
}
...
2. Evidence 已经过 Ariadne 过滤；仍然不要复述 token、密码、密钥、隐私内容。如果证据不足，直接说明不足。
