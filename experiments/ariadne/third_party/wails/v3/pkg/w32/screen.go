...
func GetAllScreens() ([]*Screen, error) {
	...
	enumFunc := func(hMonitor uintptr, hdc uintptr, lprcMonitor *RECT, lParam uintptr) uintptr {
		...
		screen := &Screen{
			...
			IsCurrent:     cursorAvailable && rectContainsPoint(monitor.RcMonitor, cursor),
		}
	...
	if len(result) > 0 && !hasCurrentScreen(result) {
		result[0].IsCurrent = true
		for _, screen := range result {
			if screen.IsPrimary {
				screen.IsCurrent = true
				if screen != result[0] {
...
func hasCurrentScreen(screens []*Screen) bool {
