...
-   **网络监控**：托盘右键一键开启网络流量监控（上传/下载）悬浮窗，支持拖拽和置顶显示；Ariadne 重构线已补充 `net mini` 贴边小窗、角落位置持久化、跟随鼠标所在屏幕/主屏/指定屏幕模式和全屏自动隐藏。
...
-   **Ariadne 重构线**：`experiments/ariadne` 已迁移为 Wails 3 + Vue 3 桌面实现，Alt+Q 主窗口保持轻量搜索器；工作记忆已接入本地摘要/标签/内容分类 enrichment、时间机器重复/相似画面合并、SQLite FTS、本地语义检索、OpenAI-compatible embedding、内置向量缓存、Milvus REST 向量存储和需二次确认的外部 AI 经验发现，并提供 `legacy <旧插件关键词>` 显式过渡入口调用旧 Python 插件。
...
当前稳定主线仍是 **Python + PyQt6**。新的 Ariadne 重构线位于 `experiments/ariadne`，采用 **Wails 3 + Go + Vue 3 + Vite + TypeScript + Pinia**，目标是把 Alt+Q 保持为干净的搜索器/启动器，同时把设置、工作记忆、Hosts、截图、剪贴板、工作流、JSON 对比和网络监控等能力拆为独立无边框工具窗口。

Ariadne 当前已能构建 Windows 桌面可执行文件，默认 Graphite Teal 浅色主题，黑色只作为手动深色模式；主 launcher 已接入插件命令补全、参数面板和受控 Python 旧插件桥，旧桥只通过 `legacy <旧插件关键词> [query]` 显式调用，不参与普通搜索召回。网络监控已有中心窗口和支持多屏模式的 `net mini` 贴边小窗，工作记忆中心已接入 Markdown/文本、图片、PDF、Office 文档和 Ariadne 导出包材料导入、按最近时间/标签/条目 ID 筛选导出、排除应用/窗口/路径/内容规则配置、截图型时间机器回放视图、支持多选证据组的独立问题复盘草稿、本地定期草稿调度，以及需要二次确认的 AI 日报润色和外部 AI 经验发现。完整进度、工具链路径、验证命令和未完成差距记录在 `docs/plans/2026-06-13-ariadne-implementation-ledger.md`，继续重构前应先读该台账，避免重复找 Go/Wails 或重复实现已完成能力。

### 跨平台演进策略

Python/PyQt 稳定主线的跨平台能力通过 `src/platform` 适配层逐步补齐，现有 Windows 行为保持不变：
