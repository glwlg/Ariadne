...
2. 不要因为没有重新 build 就判定 app 不存在；正式替换后应先查根目录 `bin\ariadne.exe`。
3. 不要重新搭 Wails/Vue 工程骨架；Ariadne 工程已从 `experiments\ariadne` 正式移到仓库根目录。
...
- 正确构建目录：`P:\workspace\glwlg\app\Ariadne`

### 最新 app 产物

- exe：`P:\workspace\glwlg\app\Ariadne\bin\ariadne.exe`
- 大小：`32568320` bytes
- 更新时间：`2026-06-17 10:10:36`
- SHA256：`62E35EC8E05750377B034987E3BB2B23B9AE3EDB0D2CB1E3BF094E38AB4740D4`
- release zip：`P:\workspace\glwlg\app\Ariadne\dist\release\ariadne-dev-windows-x64.zip`
- release zip 大小：`17007560` bytes
- release zip 更新时间：`2026-06-17 10:10:37`
- release zip SHA256：`92E0D1EA69427AAEAFEC6BC11F2A15C52F650C13720CF5A975D3532BC815426F`
- MSIX layout：`P:\workspace\glwlg\app\Ariadne\dist\msix\ariadne-0-0-0-0-msix`
- MSIX manifest：`P:\workspace\glwlg\app\Ariadne\dist\msix\ariadne-0-0-0-0-msix\msix-manifest.json`
...
- 最近 bindings：`469 Packages, 23 Services, 201 Methods, 5 Enums, 187 Models, 0 Events`
- 最近心流敏感判断降误伤：针对 2026-06-17 用户反馈“现在对敏感的判断太敏感，正常截图也判断为敏感”，确认旧 `looksSensitive` 用纯关键词匹配 `password/token/secret/验证码/密码` 等词，导致正常登录页、配置说明和验证码按钮截图被误隔离。本轮把 `workmemory` 与 `ocr` 统一到凭据形态检测：仅 `password=...`、`Authorization: Bearer ...`、Cookie/Set-Cookie、私钥块、SSH key、JWT/OpenAI/GitHub token 形态、中文 `密码/密钥/令牌/验证码: 值` 等真实凭据样式才自动置敏；普通 `密码输入框`、`token 配置说明`、`secret/API key 概念`、`验证码按钮` 不再自动敏感。`enrichEntry` 不再因为旧正文/标签关键词二次置敏，图片材料导入不再靠文件名/tag 误判；`sensitiveRulesEnabled` 设置已通过运行时 `CapturePolicy` 生效，关闭后不自动分类但手动 `Sensitive=true` 仍有效。验证：`go test ./internal/workmemory ./internal/ocr -run "Sensitive|OCR|Quality|ReviewPending" -v`、清理 AI/Embedding/OCR 环境变量后的 `go test . ./cmd/... ./internal/...`、`pnpm --dir frontend build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 `bin\ariadne.exe` 为 `32568320` bytes，时间 `2026-06-17 10:10:36`，SHA256 `62E35EC8E05750377B034987E3BB2B23B9AE3EDB0D2CB1E3BF094E38AB4740D4`；新 release zip 为 `17007560` bytes，时间 `2026-06-17 10:10:37`，SHA256 `92E0D1EA69427AAEAFEC6BC11F2A15C52F650C13720CF5A975D3532BC815426F`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面敏感截图 UI 复验。
- 最近心流时间线 OCR 补跑/重跑勾选项：针对 2026-06-17 用户要求“补跑操作改成补跑/重跑我勾选的，不要自动选择”，确认原 `timelineBatchOcrEntries` 在没有勾选时会自动回退到 `timelineEntries.filter(needsOCRSummary)`，导致按钮按当前筛选中的候选数量执行补跑。现改为只处理 `timelineSelectedEntries.filter(canRunTimelineOCR)`，不再隐式选择当前结果；按钮文案改为 `补跑/重跑我勾选的`、`补跑/重跑已勾选 N`、`无可处理勾选项`，运行态文案为 `正在补跑/重跑勾选项`，并在未勾选、无可处理项或批处理中禁用；tooltip 解释非图片、敏感或待质检记录会跳过。保留“选择当前结果”作为用户显式批量勾选入口。验证：`pnpm --dir frontend build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 `bin\ariadne.exe` 为 `32563712` bytes，时间 `2026-06-17 09:38:37`，SHA256 `C2123BCBCC790AF35EC3627FC901E37DD4D64905B8C7BCB4F81C750FEC8B3B85`；新 release zip 为 `17003755` bytes，时间 `2026-06-17 09:38:40`，SHA256 `8F3766E3D4776B406F21EBBBABA0490289871B6FBA099F241576D63993510079`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面时间线点击复验。
- 最近心流质检触发修复：针对 2026-06-16 用户反馈“感觉一直没触发质检”，确认本机 `%APPDATA%\Ariadne\work_memory.json` 里 13 条工作记忆全部仍是 `qualityStatus=pending`，`checked=0`，`lastQualityCheckAt=null`，现象属实。根因是 worker 只在一小时 `qualityTicker` 上调用 `ReviewPendingCaptures()`，且当前活跃窗口 session 会被有意跳过；如果用户一直停留在同一窗口或刚采集后查看，界面会长期显示待质检。本轮改为 worker 启动时先异步补跑一次质检，并在前台窗口签名变化时异步质检上一段已结束 pending session；新窗口的 active session 仍由 `currentWindowSessionEntryID` 保护，不会提前进入总结/检索。异步质检增加 `qualityReviewRunning` 串行保护，避免快速切窗口时并发打爆 OCR；重复/近似时间机器画面合并后也会把质量状态重新置为 pending，避免已质检旧记录被新画面合并后漏检。新增 `TestWindowChangeTriggersReviewForEndedPendingSession` 覆盖窗口切换后上一段 pending 自动变 checked。验证：`go test ./internal/workmemory -run "WindowChangeTriggersReview|ReviewPending|Quality|TimeMachineWindowSession|AutoOCR" -v`、`go test ./internal/workmemory ./internal/ocr ./internal/aiclient -v`、清理 AI/Embedding/OCR 环境变量后的 `go test . ./cmd/... ./internal/...`、`pnpm --dir frontend build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 `bin\ariadne.exe` 为 `32563712` bytes，时间 `2026-06-16 22:56:55`，SHA256 `7AEB737B4A926D301D9F3555CC21C50DA9A22BD36E4D1EFDB38468B3CDE4D44C`；新 release zip 为 `17003682` bytes，时间 `2026-06-16 22:56:56`，SHA256 `155605CD58039C31EE4CE246FC3D5E899232F6C6331093F142EC7AC8CF83AF39`。本轮只走后台命令行和本地 JSON 检查，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面质检 UI 复验。
- 最近 GLM-OCR Ollama generate OCR 兼容：针对 2026-06-16 用户已在另一台电脑用 Ollama 部署 `glm-ocr:latest` 并调通 `http://192.168.1.11:11434/api/generate`，确认 Ariadne 原实现只支持 OpenAI-compatible `/chat/completions`，不兼容 Ollama 原生视觉请求。本轮新增 OCR provider `ollama-generate` / `ollama_generate` / `ollama`，会读取图片文件后把裸 base64 放入 JSON `images` 数组发送到 Ollama `/api/generate`，不使用 multipart 文件上传，也不使用 OpenAI `image_url` data URL；`ocrBaseUrl` 可填完整 `/api/generate` 端点，也可填 Ollama 根地址或 `/api`，代码会归一化。该 provider 不要求 API key；OpenAI-compatible OCR 仍维持原来的 API key 与 `/chat/completions` 逻辑。心流设置页和设置中心文案/placeholder 已同步提示 `openai-compatible / ollama-generate`、`glm-ocr:latest` 和示例地址。验证：`go test ./internal/aiclient ./internal/ocr ./internal/settings -run "OCR|Ollama|ApplyOCRAIRuntime|Settings" -v`、清理 AI/Embedding/OCR 环境变量后的 `go test . ./cmd/... ./internal/...`、`pnpm --dir frontend build`、使用 `assets/logo.png` 对 `http://192.168.1.11:11434/api/generate` 做真实 base64 `images` smoke 并返回 `done=true`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 `bin\ariadne.exe` 为 `32561664` bytes，时间 `2026-06-16 22:33:45`，SHA256 `660C37415DA53DE7E7AC212F8E480B2325277FBC9B7FCA87304C84DA905E1173`；新 release zip 为 `17002209` bytes，时间 `2026-06-16 22:33:46`，SHA256 `D61567BCE2C179DA762A2757567E8D1B0C026BE808EEB3FF34EBE74ABC55B9CD`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面 OCR 点击复验。
- 最近心流待质检截图 OCR 预检前置：针对 2026-06-16 用户指出“心流需要先质检再做其他操作，但 OCR 不做会影响质检效果”，新增 `qualityOcrText` / `qualityOcrStatus` 作为 pending 阶段的质检 OCR 字段。pending 截图执行 OCR 时不再写入正式 `ocrText`，不会提前进入 FTS、日报、复盘、经验发现、embedding/Milvus、心流问答或 Agent evidence；`ReviewPendingCaptures()` 改为先对非活跃 pending 截图跑质检 OCR，再折叠重复帧并标记 checked，checked 后将质检 OCR 提升为正式 OCR 写回和标题/摘要整理。`workmemory.Search()` 对非空查询跳过 pending，`fts.Rebuild()` 只索引非敏感且非 pending 的可抽取记录；心流首页 OCR 健康卡区分“预检/正式”，health summary 增加 `qualityOcrDone/qualityOcrPending/qualityOcrFailed`。验证：`go test ./internal/workmemory ./internal/ocr -run "OCR|Quality|ReviewPending" -v`、清理 AI/Embedding/OCR 环境变量后的 `go test . ./cmd/... ./internal/...`、`pnpm --dir frontend build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 `bin\ariadne.exe` 为 `32547328` bytes，时间 `2026-06-16 22:23:59`，SHA256 `F6C68FFFF18430891582EC3C418DEB36D75BACC1F367CBB0A2E0C9B38A4D0DF0`；新 release zip 为 `16998396` bytes，时间 `2026-06-16 22:24:01`，SHA256 `FBABF4786B0E13954AEE19079CCAD1A4918664F63D3FCB2EA6E898EAD266F639`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面质检 OCR 视觉复验。
- 最近正式替换：针对 2026-06-16 用户要求“将 x-tools 的代码收起来，Ariadne 的代码移出来，做一个正式替换，然后提交并推送代码”，仓库根目录已切换为 Ariadne Wails3/Vue3 工程；旧 Python/PyQt x-tools 代码完整归档到 `legacy/x-tools-python`，Ariadne 源码从 `experiments/ariadne` 移到根目录，构建入口改为仓库根目录，`experiments/ariadne` 仅保留 ignored 的历史生成残留。根 `.gitignore` 同步忽略 `bin/`、`dist/`、`frontend/node_modules/`、`frontend/dist/` 和旧实验残留。Ariadne 生产代码中的 Python 旧插件 bridge 已删除，不再注册 `legacy_python` 插件或执行旧 Python 插件；旧源码只作为归档参考。验证：`go test . ./cmd/... ./internal/...`、`vue-tsc --noEmit`、`pnpm --dir frontend build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 `bin\ariadne.exe` 为 `32534528` bytes，时间 `2026-06-16 21:50:57`，SHA256 `957A2A490193A6DD658F020FD3963C601B0F195E463399521A730EF74DF6659A`；新 release zip 为 `16987699` bytes，时间 `2026-06-16 21:50:58`，SHA256 `CD5A45D44B23C783795E73627EB0737BA3A1777DF3F650FF73C469A6C898EC24`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面。
- 最近心流 OpenAI Agents SDK 默认 runner 修复：针对 2026-06-16 用户质疑“心流没有实际检索内容、默认不该走 Codex、是不是并没有通过 OpenAI Agent SDK 实现内置 agent”，确认上一轮 `openai-agent` 名称实际仍是直接 OpenAI-compatible chat completion，容易误导。本轮新增 `flow_agents_sdk_bridge.py`，默认 `openai-agent` runner 改为 Ariadne 先本地检索/过滤证据，再通过 Python OpenAI Agents SDK sidecar 调用 `OpenAIChatCompletionsModel` 生成回答；Codex 仅在 `CodexCollaborationEnabled` 或 `ARIADNE_FLOW_AGENT=codex` 显式开启时使用，`ExternalAgentEnabled` 只控制任务包生成，不再触发普通心流问答走 Codex。联系人提问加入 `跟/聊/说了什么` 意图识别和人名 evidence 排序；runner 失败或返回空内容时改为可见 `agent_error`，不再把本地统计摘要伪装成动态回答。前端文案改为 `OpenAI Agents SDK 问答` / `Agent sidecar`。本机已用系统 Python 3.13 创建 `%LOCALAPPDATA%\Ariadne\agent-python` 并安装 `openai-agents 0.17.5`，运行时默认查找该 Ariadne 自有 runtime，避免依赖旧 x-tools `.venv`，除非显式设置 `ARIADNE_FLOW_AGENT_ALLOW_REPO_PYTHON=1`。验证：`go test ./internal/aiclient ./internal/workmemory -run "FlowAgent|AskFlow" -v`、清理 AI/Embedding 环境变量后的 `go test ./...`、`vue-tsc --noEmit`、`pnpm build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；本地 mock server 无真实 key 烟测验证已安装 SDK 可运行 `Agent` + `Runner` + `OpenAIChatCompletionsModel` 并返回 `mock sdk answer`；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 `bin\ariadne.exe` 为 `32405504` bytes，时间 `2026-06-16 17:04:58`，SHA256 `DA4CD5429493D16E12777EFF25F297957BBDCFFFB509F8B33F97E721A01AFD0A`；新 release zip 为 `16934844` bytes，时间 `2026-06-16 17:05:00`，SHA256 `901B60F908A3678856B07640881B35E33B60D477C230C2DC38A4B403F3916630`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面 Flow 提问复验。
- 最近独立工具窗口路由优先与标题栏图标补强：针对 2026-06-16 用户反馈“网络小窗异常、设置界面显示为心流页面、左上角还是没显示品牌 logo、任务栏按钮已经正常”，确认根因是前端模板条件先判断共享 `appShell.activeView === 'work-memory'`，而独立窗口 `standaloneToolView` 虽然已从 `?view=settings` / `?view=network-mini` 解析正确，却被默认心流 activeView 抢先渲染。修复后新增 `shouldRenderToolView()`，独立窗口 URL view 优先于全局 activeView，`settings` 和 `network-mini` 不再误渲染心流。普通主/工具窗口 mounted 后调用 `ApplyMainWindowPolicy()`，后端 `EnableTaskbarToggle()` 复用完整普通窗口策略，不只恢复任务栏样式，还立即设置 Ariadne small/big icon 并在 80ms/300ms/900ms 后重试，规避 Wails/WebView HWND 尚未完全就绪导致标题栏仍显示默认图标。验证：`vue-tsc --noEmit`、`go test ./internal/toolwindows ./internal/shell`、`pnpm build`、清理 AI/Embedding 环境变量后的 `go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物；bindings 为 `469 Packages, 23 Services, 199 Methods, 5 Enums, 183 Models, 0 Events`。最新 `bin\ariadne.exe` 为 `32335872` bytes，时间 `2026-06-16 16:22:32`，SHA256 `DD4A23DC6589ECE7648E907567B4711C749A9265334252F13B7C001E467A6320`；新 release zip 为 `16899588` bytes，时间 `2026-06-16 16:22:34`，SHA256 `69DD3721E843328308085F2EE16E799C49A414E149C47E0759EAB7584326829A`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实设置/网络小窗/标题栏图标视觉复验。
- 最近普通工具窗口独立化和品牌图标修复：针对 2026-06-16 用户反馈“设置窗口正常但没有任务栏图标；心流窗口乱、截图后消失；心流也应该和设置一样是独立窗口，两个窗口可同时出现；窗口左上角缺品牌 logo”，确认根因是 `toolWindowsApi.ts` 使用 `@vite-ignore` + 字符串动态 import，Vite 生产包没有把 `toolwindows/service.js` binding 打入 bundle，运行时打开工具窗口失败后进入主窗口 fallback；主窗口又设置了 `HiddenOnTaskbar=true`，所以设置/心流看似普通窗口但没有独立任务栏图标，心流也会被截图流程的主窗口恢复逻辑覆盖。修复后 `toolwindows` binding 改为静态动态 import 并显式处理无 `.d.ts` 的生成文件；生产包中若工具窗口服务失败，不再静默 fallback 到主窗口；launcher 等待独立窗口打开结果再显示反馈。`work-memory` 工具窗标题改为 `Ariadne - 心流`；普通工具窗口继续保留 native titlebar/taskbar，且通过 Windows helper 对 `WM_SETICON` 设置 Ariadne small/big icon；`toolwindows.SetWindowIcon(appIcon)` 在启动时注入品牌图标。新增/更新测试覆盖普通窗口不隐藏 taskbar/icon、网速小窗继续隐藏、心流标题使用品牌名。验证：`go test ./internal/toolwindows -v`、`vue-tsc --noEmit`、`pnpm build`、清理 AI/Embedding 环境变量后的 `go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物；bindings 为 `469 Packages, 23 Services, 198 Methods, 5 Enums, 183 Models, 0 Events`。最新 `bin\ariadne.exe` 为 `32327168` bytes，时间 `2026-06-16 15:42:11`，SHA256 `547C25BD6A60A0F75734BB67825808DBE98DA8834B72CA5E0A294A955C9D20D2`；新 release zip 为 `16893943` bytes，时间 `2026-06-16 15:42:12`，SHA256 `D077DFA03D96E5EE3BF24F7BA6B4F1F261FF0274D64353059C43E5BEB205F5A2`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实任务栏图标/心流与设置并存视觉复验。
- 最近普通业务窗口恢复原生窗口框架：针对 2026-06-16 用户澄清“这个窗口就是一个无头窗口，正常窗口能做的操作它都不行；只有启动器和网络小窗这种特殊窗体才不需要头”，撤回上一轮继续补无头窗口样式的方向。`toolwindows.frameless(view)` 现在只对 `network-mini` 返回 true，设置中心、心流、历史、Hosts、工作流、JSON 对比等普通工具窗口恢复 Wails/Windows 原生标题栏、边框拖拽、任务栏点击最小化/还原和系统窗口行为；前端移除普通工具窗口的自绘 `WindowControls`，避免与原生标题栏重复；主 `Ariadne` 启动器继续无头但 `HiddenOnTaskbar=true`，避免任务栏图标落到 launcher 这个特殊窗体上。启动器、截图遮罩、贴图和网速小窗仍保留特殊无头策略，其中网速小窗继续隐藏任务栏。验证：`go test ./internal/toolwindows -v`、`vue-tsc --noEmit`、清理 AI/Embedding 环境变量后的 `go test ./...`、`pnpm build`、`wails3 task windows:package` 均通过；bindings 为 `469 Packages, 23 Services, 197 Methods, 5 Enums, 183 Models, 0 Events`。最新 `bin\ariadne.exe` 为 `32321024` bytes，时间 `2026-06-16 15:18:26`，SHA256 `796DA15631D11E3E068FE08A43BEBF3EDCEFD8E1FCD85D0E506B5691A906D82A`；新 release zip 为 `16888516` bytes，时间 `2026-06-16 15:18:27`，SHA256 `8156419DC2C0BF628B3A0C2009E245FA516E44442C32CEDD56A9B63BC668F2FF`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实任务栏点击复验。
- 最近普通工具窗口任务栏点击最小化修复：针对 2026-06-16 用户反馈“点任务栏的图标不能最小化 Ariadne，其他程序都可以”，确认第一次修复只在 Wails 运行时 `SetFrameless(true)` 生成的 `WS_POPUP` 窗口上补 `WS_SYSMENU/WS_MINIMIZEBOX` 等标志，不足以让 Explorer 任务栏按普通窗口 toggle。当前修复改为普通工具窗创建阶段即使用 Wails frameless，由 Wails `WM_NCCALCSIZE` 隐藏原生标题栏；前端普通工具窗不再重复调用运行时 `SetFrameless(true)`；`toolwindows.EnableTaskbarToggle(ctx, view)` 会恢复 `WS_OVERLAPPEDWINDOW | WS_VISIBLE`、清除 `WS_POPUP`，确保 `WS_EX_APPWINDOW`，清除 `WS_EX_TOOLWINDOW/WS_EX_NOACTIVATE/WS_EX_TOPMOST`，清空 owner 并移出 topmost z-order。启动器、截图遮罩、贴图、网速小窗不应用该样式。新增 `TestToolWindowsStayFramelessButOnlyNetworkMiniIsFixedUtility`、`TestOrdinaryTaskbarStyleRestoresOverlappedWindowSemantics`、`TestOrdinaryTaskbarExStyleUsesAppWindow` 和 `TestTaskbarToggleOnlyAppliesToOrdinaryToolWindows`。验证：`go test ./internal/toolwindows -v`、`vue-tsc --noEmit`、清理 AI/Embedding 环境变量后的 `go test ./...`、`pnpm build`、`wails3 task windows:package` 均通过；bindings 为 `469 Packages, 23 Services, 197 Methods, 5 Enums, 183 Models, 0 Events`。最新 `bin\ariadne.exe` 为 `32325120` bytes，时间 `2026-06-16 15:07:24`，SHA256 `53721642A1A6603D0956F3AAD21DF4ADF078C5F9E2D67D7DB684F9D0128C427F`；新 release zip 为 `16893366` bytes，时间 `2026-06-16 15:07:26`，SHA256 `F40E0595E0F928405D205947F9E16FFCFB607C5DE3DD7F8B26FB01A864EBCEDD`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实任务栏点击复验。
- 最近心流时间线程序筛选可搜索下拉框：针对 2026-06-16 用户要求“时间线的按程序筛选改成下拉框选择，下拉框要支持搜索”，`WorkMemoryCenter.vue` 将时间线来源程序横向 chip 列表改为自定义 combobox，下拉中保留“全部程序”和各程序计数，支持按程序展示名或内部 id 搜索，打开后自动聚焦搜索框，选择后复用原 `setTimelineAppFilter` 逻辑并重置分页/批量选择；点击外部或 Esc 关闭。`style.css` 新增下拉触发器、搜索框、选项列表、空状态和选中态样式，保持心流当前视觉系统。验证：`vue-tsc --noEmit`、`pnpm build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 196 Methods, 5 Enums, 183 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32318464` bytes，时间 `2026-06-16 14:35:15`，SHA256 `94AAB8E7B6ACFF161759D285945E69E23115738F686D1463C848147944C02986`；新 release zip 为 `16884506` bytes，时间 `2026-06-16 14:35:16`，SHA256 `CD24BB2DAE27039F02D7194124BE23530B84622CE4C816B9AC645B6DB071EE3A`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面下拉筛选点击复验。
- 最近心流时间线批量 OCR+总结：针对 2026-06-16 用户要求“已有数据很多没有 OCR，增加批量 OCR 按钮，注意是 OCR+总结并加进度条”，心流时间线新增 `批量 OCR+总结` 按钮。已勾选轨迹时处理勾选项，未勾选时处理当前筛选下有图片证据且缺 OCR 或标题/摘要仍为泛化截图记录的条目；批量逻辑逐条调用现有 `RecognizeWorkMemory -> ApplyOCRText` 链路，因此会执行 OCR 写回、本地标题/摘要/正文整理，以及启用 AI 时的 OCR 总结。敏感记录和待质检记录默认跳过。新增独立 OCR 进度状态和进度条，显示 `done/total` 与当前处理对象，删除进度互不干扰。验证：`vue-tsc --noEmit`、`pnpm build`、`go test ./internal/ocr ./internal/workmemory -run "OCR|ApplyOCR|RecognizeWorkMemory" -v`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 196 Methods, 5 Enums, 183 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32315392` bytes，时间 `2026-06-16 14:30:13`，SHA256 `9D04ACE8F7CFEC9D70F4A021919753823B2F1B5220398D95B30771F1294FA03F`；新 release zip 为 `16883325` bytes，时间 `2026-06-16 14:30:14`，SHA256 `DF146720185BD36A241E492F025901AAD777D197455B9BE2BB534ACC04CF5BEC`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面批量 OCR 点击复验。
- 最近心流自主沉淀收件箱与默认采纳机制：针对 2026-06-16 用户要求“心流需要更高自主性，证据足够就自动生成 Skill/知识/日报/复盘，通知我即可，不删除就默认采纳，删除可输入原因避免下次再生成”，`workmemory.Service` 新增自主产物、拒绝原因台账和每日自主流程。自主流程只读取非敏感、已质检证据，按策略生成日报、复盘、知识和低风险 Skill 产物；低风险 Skill 仅在证据足够、流程清晰且所有步骤均无需确认时生成，并标记 `agentExecutable`。删除产物时记录 `dedupKey` 和用户原因，后续同类产物被抑制；心流首页新增“自主沉淀”轻量收件箱和“自主整理”入口，未删除即默认采纳，支持复制产物正文和删除时输入原因。设置版本迁移到 v11，旧配置会默认开启每日定时草稿/自主检查，但时间机器本身仍默认关闭。验证：`go test ./internal/workmemory ./internal/settings -v`、`vue-tsc --noEmit`、清理 AI/Embedding 环境变量后的 `go test ./...`、`pnpm build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 196 Methods, 5 Enums, 183 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32312320` bytes，时间 `2026-06-16 14:17:58`，SHA256 `F47336D76D84766053ACF8806C67491DCFCDB888E6C799F0DA593FEA11FFB7BD`；新 release zip 为 `16882643` bytes，时间 `2026-06-16 14:17:59`，SHA256 `ECF775B7694E5C5EE305AAD126AB7E3059258D41295861DB892940B3586EF3E3`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面收件箱点击复验。
- 最近心流时间线截图 OCR 自动整理：针对 2026-06-16 用户反馈“时间线标题没意义、点进去 OCR 都没做、截图后应自动 OCR 并 AI 总结标题和整齐文本”，新配置默认开启 `autoOcr`；截图类工作记忆质检通过后自动 OCR，`ApplyOCRText` 回写时生成整理后的 `title/summary/text`，替换 `work`、`截图证据`、`屏幕时间机器` 等泛标题，并把路径/尺寸/来源等噪声过滤掉。AI 配置启用时通过 `OpenAICompatibleOCRSummarizer` 复用 Ariadne AI provider/base/model 和 Credential Manager/API env key 生成更自然的标题、摘要和 Markdown 正文；隐私模式、敏感条目、无模型或 AI 失败时保留本地整理兜底。心流时间线优先展示整理字段，原始 OCR 折叠为“原始 OCR 证据”。验证：`go test ./internal/workmemory ./internal/aiclient ./internal/settings -v`、`vue-tsc --noEmit`、`pnpm build`、清理 AI/Embedding 环境变量后的 `go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 193 Methods, 5 Enums, 179 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32207360` bytes，时间 `2026-06-16 13:35:42`，SHA256 `34C1731C786AB39806BDA7544BBB4AF69F212680201DB98DDBDB4AA0D1FAC070`；新 release zip 为 `16841214` bytes，时间 `2026-06-16 13:35:43`，SHA256 `400D2D785BD6920A1B9F414A16BD6973F6CC3BF78585FE7329E765932B8C2C74`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面时间线/OCR视觉复验。
- 最近网速小窗启动恢复与截图工具栏顺序调整：针对 2026-06-16 用户反馈“每次启动都要重新开启网络小窗、截图工具栏的颜色和下面几个高频操作按钮换位置”，`network_mini_window.json` 新增 `visible` 状态，用户打开网速小窗后会持久化，下次 Ariadne 启动时 `toolwindows.Attach()` 自动恢复；`ResetNetworkMiniPlacement()` 保留原可见状态，避免重置位置后丢失启动恢复。截图覆盖层工具栏把 `保存 / 另存 / 复制 / 贴图` 移到原颜色区，颜色选择和粗细控制移到下面原高频操作区。新增 `TestNetworkMiniVisibleStatePersists` 和 `TestNetworkMiniResetPlacementPreservesVisibleState`。验证：`vue-tsc --noEmit`、`go test ./internal/toolwindows`、`pnpm build`、清理 AI/Embedding 环境变量后的 `go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物；bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`。新 `bin\ariadne.exe` 为 `32124928` bytes，时间 `2026-06-16 13:14:51`，SHA256 `DA5F56ED364F9122CAFBAB368BDF4487D6E926F3A0516E93BE152730351E5C34`；新 release zip 为 `16787230` bytes，时间 `2026-06-16 13:14:53`，SHA256 `46DAA73C287CCD0F702F6E62451D1821D544414B55C1AB0F552FA782EDD5786E`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面启动恢复/工具栏点击复验。
- 最近时间机器默认采集节流、自动质检与未质检隔离：针对 2026-06-16 用户反馈“自动截图要先等 3 秒、驻留 30 秒、多张作为一次采集、每小时自检清垃圾、总结/提取不处理未质检记录”，时间机器默认策略改为窗口切换后稳定 `3s` 才采集，窗口保持期间默认 `30s` 再采集一次；不到 3 秒就切走的窗口不落库。相同窗口驻留期间的多次截图会追加到同一条 `time_machine` 记忆的 `frames`，记录保持 `qualityStatus=pending`，不会进入 OCR、日报、复盘、经验发现、embedding/Milvus、心流问答和 Codex agent evidence。worker 新增每小时 `ReviewPendingCaptures()` 自检，跳过当前仍活跃的窗口会话，已结束的 pending 记录会折叠重复/近似重复帧，只保留最新帧并标记 `checked`；质检通过后才允许自动 OCR。当前已实现像素签名/保守感知指纹去重，尚未实现“最后一张语义上包含前面信息”的 OCR/视觉语义判断。默认设置和前端 fallback 同步为 `autoCaptureIntervalSeconds=30`、`windowSwitchCaptureEnabled=true`、`windowSwitchCooldownSeconds=3`，应用级采集策略仍可单独覆盖。新增/更新测试覆盖 3 秒稳定等待、驻留多帧合并成一条记录、自动质检折叠重复帧、pending 不进入日报/工作流草稿/心流回答/Codex agent evidence，以及质检后才触发自动 OCR。验证：`go test ./internal/workmemory ./internal/settings`、清理 AI 环境变量后的 `go test ./...`、`pnpm build`、`wails3 task windows:package` 均通过；未清理 shell 中的 AI key 时 `internal/aiclient` 的“缺 key”测试会被环境变量污染并收到接口 401，清理后通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物；bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`。新 `bin\ariadne.exe` 为 `32108032` bytes，时间 `2026-06-16 10:58:48`，SHA256 `C58D61B5F0357EC178CB88E302918FD82C2D23174F86F8AA29970036D9143AED`；新 release zip 为 `16781228` bytes，时间 `2026-06-16 10:58:49`，SHA256 `AFDE8D059E2EFF5C2E26575F95BF4A69E11B0E0554CF5FF69BD3E674576B4744`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面采集/质检视觉复验。
- 最近心流问答后端契约和首页接入：针对 2026-06-15 用户要求“继续完善心流的功能”，`internal/workmemory.Service.AskFlow` 新增统一问答入口，支持“今日总结 / 谁找过我 / 工作流优化 / 普通检索”四类意图识别，回答只使用本地非敏感记忆，统一返回 answer、intent、mode、suggestedQuestions 和本次问题对应的 evidence；`workMemoryApi.ts`、`stores/workMemory.ts` 和 `WorkMemoryCenter.vue` 改为调用该入口，不再在组件里分散触发日报、语义搜索和经验发现；证据抽屉优先展示本次问答命中的记忆，回答复制文本和回答正文子元素显式 `no-drag`/`user-select:text`，避免拖选文字时拖动窗口。新增 `TestAskFlowSummarizesTodayAndSkipsSensitiveEvidence` 和 `TestAskFlowSummarizesCommunicationContext`，覆盖今日总结、沟通类问题和敏感记录不进入回答/证据。验证：`go test ./internal/workmemory -count=1`、`go test ./...`、`pnpm build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物；bindings 为 `469 Packages, 23 Services, 190 Methods, 5 Enums, 173 Models, 0 Events`。新 `bin\ariadne.exe` 为 `31979008` bytes，时间 `2026-06-15 23:50:03`，SHA256 `2E65377CFBA76566F8C4F1FC724553B84B37BBB730D5549E00F826C49A9F9551`；新 release zip 为 `16728728` bytes，时间 `2026-06-15 23:50:04`，SHA256 `115EDA3D2492384A9C6644144953DB521A3434B0EF38D66350140A92BA44047B`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面问答点击复验。
- 最近心流时间线和设置重排：针对 2026-06-15 用户反馈“时间线页面就是把原来的工作记忆中心塞进来了、乱糟糟没有主次、心流设置做得敷衍且叠压”，`WorkMemoryCenter.vue` 删除时间线页旧三栏工作记忆结构，改为时间线 header、统计、来源筛选、最近轨迹列表和右侧轻量复盘/时间机器/今日来源摘要；明细、OCR 图片和文本证据默认不常驻，点击轨迹后才打开证据抽屉；手动笔记、语义索引、数据包、排除规则、日报/知识/任务包等低频模块留在草稿/资产/规则等独立页。心流设置抽屉改为状态概览 + 采集/模型/边界三段页签，抽屉结构改为 header/body/footer，body 独立滚动，修复原 grid 布局下多个配置 section 叠压的问题；`style.css` 新增 `flow-timeline-*`、`flow-quiet-panel`、`flow-settings-overview/tabs/body` 和窄屏规则。验证：`pnpm build`、`go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物；bindings 为 `469 Packages, 23 Services, 189 Methods, 5 Enums, 170 Models, 0 Events`。新 `bin\ariadne.exe` 为 `31920128` bytes，时间 `2026-06-15 22:56:54`，SHA256 `2DEA34B0B4410BAC3AA63B09F3EE43C1EE0624F1A660E5846850B71FA1A7A7E7`；新 release zip 为 `16697550` bytes，时间 `2026-06-15 22:56:54`，SHA256 `6A9B878B0388F686DCECCA1977E46BB71737DD3022AAD5E5DF56284053802A2B`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面视觉复验。
- 最近心流侧栏、回答复制、心流设置抽屉和窗口缩放修复：针对 2026-06-15 用户反馈“侧边菜单没有字/logo 缺失、心流回答没有叙事结构且不能复制、鼠标拖选会拖动窗口、窗口除最大化外不能拖拽边框调节大小”，`WorkMemoryCenter.vue` 将侧栏恢复为带 Ariadne 真实图标、文字和副标题的品牌导航，去掉窄窗口下自动折叠为纯图标栏；心流回答改为可复制的摘要段落 + 主线/证据/洞察三段结构，新增“复制回答”按钮，回答内容区域标记为 `no-drag` 并开启文本选择；心流侧栏“设置”不再跳全局设置中心，改为本页设置抽屉，集中维护采集、自动整理、AI/Embedding/Milvus、隐私排除和存储保留策略；设置中心可见导航移除工作记忆、AI 与向量、隐私规则入口，数据页不再放心流保留配置。`App.vue`、`appShell.ts` 和 `main.go` 调整普通工具窗口 resize 模式，启动器/截图遮罩/贴图/网速小窗仍保持特殊窗口策略，普通工具视图恢复边框拖拽缩放能力。验证：`pnpm --dir experiments\ariadne\frontend exec vue-tsc --noEmit`、`pnpm --dir experiments\ariadne\frontend build`、`go test ./internal/toolwindows -v`、`go test ./...`、`wails3 task windows:package` 均通过；当前 shell 的 `go`/`wails3` 不在 PATH，使用固定便携工具链 `C:\Users\luwei\.codex\tools\go1.26.4.windows-amd64\go\bin\go.exe` 和 `C:\Users\luwei\.codex\go-bin\wails3.exe`；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物；bindings 为 `469 Packages, 23 Services, 189 Methods, 5 Enums, 170 Models, 0 Events`。新 `bin\ariadne.exe` 为 `31934464` bytes，时间 `2026-06-15 22:36:46`，SHA256 `A545B6DB85BA4716A8A7BA0B064F4708859F40FBEA8F24B4617DEB4EC6617CC7`；新 release zip 为 `16699803` bytes，时间 `2026-06-15 22:36:47`，SHA256 `85BF1F7B4F485499A328E495659B8FE7975582E399FB433F3D1E33B2E8F1307D`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面；因此不声明真实桌面视觉/拖拽复验。
...
- 最近旧 Python 内置插件 Go 主路径覆盖审计：`TestLegacyPythonBuiltinsHaveNativeGoCoverage` 现在枚举 `legacy/x-tools-python/src/plugins/*.py`，要求 16 个旧内置插件全部映射到 Ariadne 原生 Go manifest 或工具窗口入口：calculator、timestamp、base64、hash、json、json_compare、url、uuid、custom_launch、qr、system_commands、hosts、clipboard、capture_history、workflow、work_memory。正式替换后 Python legacy bridge 已从生产代码删除；该测试只用于防止旧能力被遗漏。
...
21. 心流动态 agent 回答验证
   - `workmemory.Service.AskFlow` 不再只返回静态本地汇总；现在会先选择非敏感 evidence、生成本地兜底答案，再通过 `FlowAgentRunner` 生成最终回答。
   - 当前已接入 `CodexFlowAgent`，通过隐藏子进程运行 `codex exec --ignore-user-config --ephemeral --skip-git-repo-check --sandbox read-only --config approval_policy="never"`，stdin 只传 question、intent、本地兜底摘要和非敏感 evidence JSON，不传截图文件路径或图片文件。
   - Flow agent 策略由 AI/Codex 相关设置或 `ARIADNE_FLOW_AGENT=codex` 启用；`ARIADNE_FLOW_AGENT=off` 可强制关闭，`ARIADNE_FLOW_CODEX_MODEL` 可指定 Codex 模型。Agent 失败或超时时会保留本地答案并在 message 中说明回退原因。
   - 后台 Codex 探针已验证 `codex exec` 可用：轻量参数下返回 `ariadne-flow-agent-light-ok`，耗时约 `13.7s`。该探针只走命令行，没有控制桌面。
   - 验证命令：`go test ./internal/workmemory -run "AskFlow|FlowAgent" -v`、`go test ./internal/aiclient -v`、`go test ./...`、`wails3 generate bindings -clean=false`、`pnpm build`、`wails3 task windows:package` 均通过。最新 bindings 为 `469 Packages, 23 Services, 191 Methods, 5 Enums, 174 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32009728` bytes，SHA256 `8C6A79DD9E68C87E56C47BD7F4682B31092F4F63A446A1A1FEA573AD334F759D`；release zip 为 `16740142` bytes，SHA256 `E07F01F77D6B3FB66473C7A95275CE5B45CBD8C55A6784CF449C7D1EF6BFE1EB`。
22. 心流首页对话框化与显式沉淀
   - `WorkMemoryCenter.vue` 首页从大提问卡、静态回答卡和右侧看板，改为“我与心流的对话”单列聊天线程。
   - 对话消息默认仅保存在当前前端会话，不自动写入工作记忆。
   - 消息支持右键菜单、勾选/多选、复制选中和“加入沉淀”；显式沉淀复用 `AddNote` 后端入口，经 `workMemory.addManualNote()` 以 `manual_note` 写入，并打上 `心流`、`对话沉淀` 标签。
   - 首页证据抽屉保留但默认收起，右侧摘要/洞察/模型卡片从首页移除，避免主交互继续像看板；消息文本、气泡和聊天线程显式 `no-drag` 与 `user-select: text`，右键菜单使用 fixed 层避免被气泡裁剪。
   - 验证命令：`pnpm build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 191 Methods, 5 Enums, 174 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32012800` bytes，SHA256 `72A41066FC41157C506EF272130AAC67B5AA5B0683B88A9AFFBCDA3EBA098FEF`；release zip 为 `16740458` bytes，SHA256 `193A0688E9214DAB9147633F1314E247A2B3F0053088531C7D771213D3BF821E`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面视觉/右键交互复验。
23. 心流聊天布局与设置去重/密钥补回
   - 修复 `.flow-message` grid 列定位：assistant/system 消息即使没有选择按钮，也固定使用头像列 + 气泡列，避免气泡落入 44px 窄列导致文字逐字换行。
   - 心流设置“隐私边界与存储”页签移除排除规则文本区，排除应用、窗口关键词、路径、URL 和内容正则仅保留在“规则”页面维护，避免双入口状态混乱。
   - 心流设置“模型与向量”页签补回安全密钥存储，复用 settings store 的 `secretStatus`、`secretInputs`、`saveSecret` 和 `clearSecret`，支持 AI API key、Embedding key、Milvus token 的保存/清除、环境变量优先提示和 Credential Manager 状态展示。
   - 验证命令：`pnpm build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 191 Methods, 5 Enums, 174 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32014336` bytes，SHA256 `4205C6DB49227BD8AFCA6AD5503382702103AFBF82542F2ED652CF220744C752`；release zip 为 `16741217` bytes，SHA256 `0681BA4B8CC9D70F42FE3FAE83FBB605A863C8FE8FBD42A039EBCFC4BC731776`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面视觉/密钥保存点击复验。
24. 时间机器应用级采集策略
   - 新增 `WorkMemoryAppCaptureProfile` 配置模型，settings 版本升至 10；配置项包含 `displayName`、`processName`、启用状态、切换窗口后延迟截图秒数和窗口保持期间采集间隔，并做进程 basename 归一化、去重和边界收敛。
   - 时间机器 worker 新增应用策略接管逻辑：命中应用策略时跳过全局定时截图，由该应用自己的切窗延迟和驻留间隔触发；应用策略只依赖时间机器总开关，不强制依赖全局“窗口切换触发”，因此可单独为 Weixin.exe 这类应用定制节奏。
   - 心流设置“采集”页新增“应用采集策略”区块：已配置应用以头像/应用名/进程展示，点击后配置延迟和驻留间隔；新增应用从最近采集到的应用候选中选择，不再像排除规则一样手输进程名。当前头像为应用名首字母/首字符占位，真实 exe 图标提取仍可后续补。
   - 新增/更新测试覆盖 settings 归一化持久化、应用策略延迟切窗截图、命中应用策略时跳过全局间隔，避免微信切到私人空对话后仍被全局 120 秒截空窗。
   - 验证命令：`go test ./internal/settings ./internal/workmemory -v`、`pnpm build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 191 Methods, 5 Enums, 176 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32053760` bytes，时间 `2026-06-16 10:28:32`，SHA256 `2F398E3FEE29F5941F480DC6FA2BB20E62D9B236C17544216FC980302557100B`；release zip 为 `16761361` bytes，时间 `2026-06-16 10:28:32`，SHA256 `14EC026856CD162A46EC91A32551C8F809BD14636708380E0789017EA1DEDF91`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实微信/心流设置点击复验。
25. 时间机器稳定窗口采集、小时级质检与 pending 隔离
   - 默认时间机器采集改为稳定窗口策略：窗口切换后先等待 `3s`，未满 3 秒切走则不采；窗口保持期间默认每 `30s` 再采一次。
   - 同一窗口会话内的多次 `time_machine` 截图追加到同一条工作记忆的 `frames`，记录保持 `qualityStatus=pending`，避免把驻留窗口刷成多条时间线记录。
   - worker 新增每小时 `ReviewPendingCaptures()`：跳过当前仍活跃窗口会话，已结束 pending 记录会折叠重复/近似重复帧、只保留最新帧、标记 `checked`，并在质检后才允许自动 OCR。当前只实现签名/保守感知指纹去重；“最后一张语义上包含前面信息”仍需后续 OCR/视觉语义判断。
   - 所有总结、提取和外部证据入口改为跳过 pending：日报/问题复盘/定期草稿、经验发现、workflow/checklist/knowledge/task-package evidence、embedding/Milvus refresh、心流问答和 Codex flow agent evidence 都只使用非敏感且已质检的记录。
   - 默认设置、前端 fallback 和状态同步为 `autoCaptureIntervalSeconds=30`、`windowSwitchCaptureEnabled=true`、`windowSwitchCooldownSeconds=3`；应用级采集策略仍可单独覆盖。
   - 验证命令：`go test ./internal/workmemory ./internal/settings`、清理 AI/Embedding 环境变量后的 `go test ./...`、`pnpm build`、`wails3 task windows:package` 均通过；未清理 shell 中的 AI key 时 `internal/aiclient` 的缺 key 用例会被环境变量污染并收到接口 401，清理后通过。最新 bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32108032` bytes，时间 `2026-06-16 10:58:48`，SHA256 `C58D61B5F0357EC178CB88E302918FD82C2D23174F86F8AA29970036D9143AED`；release zip 为 `16781228` bytes，时间 `2026-06-16 10:58:49`，SHA256 `AFDE8D059E2EFF5C2E26575F95BF4A69E11B0E0554CF5FF69BD3E674576B4744`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面采集/质检视觉复验。
26. 心流聊天建议按钮与输入框点击区域修复
   - 首页聊天区的建议问题不再复用旧 ask-card 的 `.flow-question-chips` 大尺寸样式，新增 `.flow-chat-question-chips`，按钮恢复为紧凑 pill，避免变成夸张圆形/椭圆按钮。
   - 聊天输入容器新增 pointer-down 聚焦行为，点击输入框外观区域任意空白处都会聚焦 textarea；textarea 改为撑满输入框高度，避免视觉上是大输入框但只有底部文字行可点击。
   - 验证命令：`pnpm build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32109056` bytes，时间 `2026-06-16 11:07:59`，SHA256 `F4B14CD75E389E8B0B50A46C48743DC668BB4B177B2B10EE68D52CD05BF996A2`；release zip 为 `16781282` bytes，时间 `2026-06-16 11:08:00`，SHA256 `3A9279C774B2042ECF0FDA53F4BAB56A1306A22B0DBB18C5FD5149CCBC76915E`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面点击复验。
27. 心流时间线批量删除与来源程序筛选
   - 时间线页新增来源程序筛选条，基于当前来源类型筛选后的 `appName` 聚合为应用 chip，支持“全部程序”和单个来源程序叠加筛选，程序过多时横向滚动，不挤压时间线内容。
   - 时间线行左侧选择改为时间线批量选择语义，不再和复盘证据选择混用；顶部新增“选择当前结果 / 删除选中 / 清空”，删除采用二次确认，支持敏感记录删除。
   - 复盘能力保留在右侧辅助区，新增“选中入复盘”和“当前入复盘”，复盘仍只接收非敏感记录且最多 12 条。
   - store 新增按指定 ID 进入复盘和批量删除方法，批量删除复用现有 `Delete(id)` 后端入口逐条执行，完成后同步本地 entries、选中项、复盘选择、图片预览和搜索结果。
   - 验证命令：`vue-tsc --noEmit`、`pnpm build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32113152` bytes，时间 `2026-06-16 11:16:12`，SHA256 `ED88F4C8F39C53CE3F3ACE7DB60DC929D54001CF150A1F8A21EE3C7EDAC83252`；release zip 为 `16782134` bytes，时间 `2026-06-16 11:16:14`，SHA256 `889D8FC734F3E7A4551F0E95131F69035A25223CF94DF241F7075575800D79A2`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面点击复验。
28. 心流时间线 Product Design 重排与按天滚动加载
   - 按 Product Design brief 将时间线从普通列表改为按天分组的真实时间轴：左侧 day marker 展示“今天/昨天/日期”和当日轨迹摘要，右侧事件卡沿纵向轴线排列，支持窄屏退化为单列。
   - 事件主标题不再直接展示“截图历史自动沉淀”等来源描述，而是从摘要、窗口标题、OCR、文本中抽取更接近“当时在处理什么”的重点；来源程序、来源类型、窗口标题和证据类型降级为元信息 chip/badge。
   - 时间线默认只加载前 2 天，底部 IntersectionObserver 哨兵在滚动接近时按天追加加载，也保留“立即加载”按钮；来源类型或来源程序筛选变化会重置到前 2 天。
   - 批量删除、来源程序筛选和复盘选择继续保留：行选择仍是批量选择，删除仍二次确认，复盘入口在右侧辅助区。
   - 验证命令：`vue-tsc --noEmit`、`pnpm build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32118272` bytes，时间 `2026-06-16 11:23:29`，SHA256 `40B4C4B76F5AF01CF2CDB2183DBEE8803C7E090BC643DD98EBF04C6DA302757F`；release zip 为 `16784964` bytes，时间 `2026-06-16 11:23:31`，SHA256 `E9A793AD1314B7DD321EBB5D8D56FFDD81248A0AB26DBB182BDDDAD852D165D0`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面滚动/点击复验。
29. 心流进度反馈与 ChatGPT 式首页比例调整
   - AI 归纳从只改变按钮 loading 改为显式阶段进度：确认外部 AI 归纳后显示“准备非敏感证据 / AI 正在归纳线索 / 整理归纳结果”等阶段文字和进度条，完成或失败都会清理进度状态，避免界面卡住。
   - 时间线批量删除新增 done/total 进度状态：选中记录较多时，页面显示正在删除的进度条和已完成/总数，删除结束后短暂停留结果再自动收起。
   - 心流首页按 ChatGPT 网页比例重新收敛：主体变为居中的对话列，去掉大看板式卡片框架，输入框贴近聊天 composer 的高度和位置，线程区域承担主视觉，底部输入区 sticky，避免之前空白输入框和内容比例失衡。
   - 修正 AI 归纳异常收尾顺序：请求失败时先解除 `isDiscoveringExperienceAI` 再停止进度，确保进度条不会因为异常分支遗留。
   - 验证命令：`vue-tsc --noEmit`、`pnpm build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32121344` bytes，时间 `2026-06-16 11:35:53`，SHA256 `5412427049E665F4DDC7B0BFE673E5F2F110C2EAEBDC7484FAC925D5C298CB03`；release zip 为 `16786091` bytes，时间 `2026-06-16 11:35:54`，SHA256 `917F2F019E6FF95BBA6254266EB5C6427F55C010FCF1A81DEE2ABB288E58E20A`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面视觉/点击复验。
30. 心流洞察转化动作可解释与生成后跳转处理
   - 洞察卡片三个按钮从 `转任务包 / 转工作流 / 转清单` 改为 `交给代理 / 生成自动化 / 生成检查清单`，并补 title 说明，明确它们是把洞察转成不同资产，而不是接受/拒绝。
   - 三个动作生成成功后自动切到 `资产` 页并高亮对应资产卡，避免用户停留在洞察页看不到结果；资产页顶部显示轻量反馈，不使用系统级通知。
   - 外部代理卡补“下一步”说明、证据/边界/验收数量和 `复制任务包` 操作，复制出的 Markdown 包含目标、上下文、证据、边界和验收标准，可贴给 Codex 或其他代理处理。
   - 工作流和检查清单卡补生成后的下一步提示，仍需用户二次确认后才保存为正式工作流或清单。
   - `workMemory` store 的生成函数返回成功/失败；工作流/清单生成后调用 `SetExperienceInsightDecision` 保存 `workflow_draft` / `checklist_draft` 状态，后端状态归一化已支持这两个值，洞察页会显示 `已生成自动化草稿` / `已生成检查清单`。
   - 验证命令：`vue-tsc --noEmit`、`go test ./internal/workmemory`、`pnpm build`、`go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32124928` bytes，时间 `2026-06-16 12:27:45`，SHA256 `11B0CF47D1AA697D6F7485ABCE304AB1D682B72765A46F6977ED68DB430D9C94`；release zip 为 `16787088` bytes，时间 `2026-06-16 12:27:46`，SHA256 `76602492634F0A89170B270CB1CBF1D3F720A098CB6E0832C91674613ED1B4A5`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面点击复验。
31. 网速小窗启动恢复与截图工具栏顺序调整
   - `internal/toolwindows` 的 `networkMiniConfig` 增加 `visible` 字段；用户打开 `net mini` 后 `markNetworkMiniVisible()` 会写入 `network_mini_window.json`，`Attach()` 启动时读取并恢复网络小窗，解决每次启动都要手动再开的问题。
   - `ResetNetworkMiniPlacement()` 保留原 `visible` 状态，只重置位置/锚点/尺寸等布局字段，避免“重置位置”误伤启动恢复。
   - `NetworkMiniStatus` 和前端 API 类型补出 `visible`，后续设置或诊断页可以直接显示当前可见持久化状态。
   - 截图覆盖层工具栏顺序调整：`保存 / 另存 / 复制 / 贴图` 移到原颜色选择区域，颜色选择和粗细控制移到下方原高频操作区，符合当前鼠标在选区右下附近时优先操作保存/复制/贴图的路径。
   - 新增 `TestNetworkMiniVisibleStatePersists` 和 `TestNetworkMiniResetPlacementPreservesVisibleState`，覆盖可见状态持久化和重置不清除可见状态。
   - 验证命令：`vue-tsc --noEmit`、`go test ./internal/toolwindows`、`pnpm build`、清理 AI/Embedding 环境变量后的 `go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 192 Methods, 5 Enums, 178 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32124928` bytes，时间 `2026-06-16 13:14:51`，SHA256 `DA5F56ED364F9122CAFBAB368BDF4487D6E926F3A0516E93BE152730351E5C34`；release zip 为 `16787230` bytes，时间 `2026-06-16 13:14:53`，SHA256 `46DAA73C287CCD0F702F6E62451D1821D544414B55C1AB0F552FA782EDD5786E`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面启动恢复/工具栏点击复验。
32. 心流时间线截图 OCR 自动整理
   - 新配置默认开启 `WorkMemory.AutoOCR`，前端 fallback 同步为 `autoOcr=true`；已有用户配置仍以持久化值为准，不强行覆盖。
   - `workmemory.Service.ApplyOCRText` 不再只把 OCR 原文塞进记录，而是在回写时生成整理层：替换 `work`、`截图证据`、`屏幕时间机器`、`截图尚未识别` 等泛标题，清理路径/尺寸/来源等采集噪声，写入可读 `title`、一句话 `summary` 和结构化 `text`；原始 OCR 仍保留在 `ocrText` 作为证据。
   - 新增 `OCRSummaryPolicy` / `OCRSummarizer`，主程序注册 `OpenAICompatibleOCRSummarizer`，复用 Ariadne AI provider/base/model 和 Windows 环境变量/Credential Manager 的 API key；AI 启用且非隐私/非敏感时调用 chat completion 生成更自然的标题、摘要和 Markdown 正文。隐私模式、敏感条目、无模型或 AI 失败时保留本地整理兜底。
   - 心流时间线主标题和摘要展示顺序改为优先 `title/summary/text`，raw `ocrText` 降为最后 fallback；明细抽屉把原始 OCR 收进“原始 OCR 证据”折叠区，避免默认展示乱糟糟 OCR 内容。
   - 新增 `TestApplyOCRTextBuildsCleanTimelineText`、`TestApplyOCRTextUsesAISummarizerWhenConfigured` 和 `TestOpenAICompatibleOCRSummarizerPostsChatCompletion`，覆盖本地整理、AI 整理策略和 OpenAI-compatible 请求解析；旧 embedding 缺 key 测试补 `t.Setenv("EMBED__API_KEY", "")`，避免本机环境变量污染。
   - 验证命令：`go test ./internal/workmemory ./internal/aiclient ./internal/settings -v`、`vue-tsc --noEmit`、`pnpm build`、清理 AI/Embedding 环境变量后的 `go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 193 Methods, 5 Enums, 179 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32207360` bytes，时间 `2026-06-16 13:35:42`，SHA256 `34C1731C786AB39806BDA7544BBB4AF69F212680201DB98DDBDB4AA0D1FAC070`；release zip 为 `16841214` bytes，时间 `2026-06-16 13:35:43`，SHA256 `400D2D785BD6920A1B9F414A16BD6973F6CC3BF78585FE7329E765932B8C2C74`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面时间线/OCR视觉复验。
33. 心流主窗口化与启动器附属化
   - Wails `main` 窗口不再是启动器：默认创建为普通原生 `Ariadne - 心流` 主窗口，尺寸 `1280x820`，最小 `1040x640`，启用原生标题栏、任务栏按钮和 Ariadne titlebar icon，解决任务栏图标必须等截图操作后才出现的结构性问题。
   - `toolwindows.ShowLauncher()` 不再复用 `main`，而是创建/恢复独立 `tool-launcher`，URL 为 `/?view=launcher`；该窗口继续保持透明、无头、隐藏任务栏，仅承担 Alt+Q 搜索器职责。
   - `toolwindows.Open("work-memory")` 改为恢复并聚焦 `main` 心流窗口；设置、剪贴板、截图历史、Hosts、工作流、JSON 对比、网络监控等仍作为普通独立工具窗口打开，可与心流同时存在。
   - shell 生命周期重定向到心流：二次启动和托盘单击默认打开心流；托盘菜单保留“打开启动器”，并把旧“工作记忆中心”标签改为“心流”。
   - 前端路由分流同步：无 `view` 参数渲染心流主窗口并执行普通窗口运行时设置，`?view=launcher` 才渲染启动器；启动器打开工具后隐藏自身，不关闭心流或设置窗口。
   - 验证命令：`vue-tsc --noEmit`、`go test ./internal/toolwindows ./internal/shell`、`pnpm build`、清理 AI/Embedding 环境变量后的 `go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 199 Methods, 5 Enums, 183 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32334336` bytes，时间 `2026-06-16 16:11:19`，SHA256 `49675D8E80B1C5CC4AA20A0F8AA75F458914C20191E17237F99013D58EB69189`；release zip 为 `16898132` bytes，时间 `2026-06-16 16:11:20`，SHA256 `0F2DCA64F1A082623EB5639EB8BA3162919E03B6663C65031F446E57FB1CDD5F`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面任务栏点击/图标视觉复验。

34. 独立工具窗口路由优先与标题栏图标补强
   - 修复设置窗口和网络小窗误显示心流页：前端模板此前先命中 `appShell.activeView === 'work-memory'`，独立窗口的 `?view=settings` / `?view=network-mini` 虽然解析正确但不会生效；新增 `shouldRenderToolView()` 后，存在 `standaloneToolView` 时只按 URL view 渲染。
   - 补强左上角品牌图标：普通窗口 mounted 后主动调用后端 `ApplyMainWindowPolicy()`；`EnableTaskbarToggle()` 不再只修 taskbar style，而是复用完整普通窗口策略，包含 `WM_SETICON` 设置 Ariadne small/big icon 和延迟重试，避免 HWND 尚未就绪时丢失标题栏图标。
   - 保持任务栏按钮行为不回退：普通窗口仍走原生 titlebar/taskbar，启动器和网络小窗仍是特殊窗口；本轮未重新引入普通窗口自绘标题栏。
   - 验证命令：`vue-tsc --noEmit`、`go test ./internal/toolwindows ./internal/shell`、`pnpm build`、清理 AI/Embedding 环境变量后的 `go test ./...`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 199 Methods, 5 Enums, 183 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32335872` bytes，时间 `2026-06-16 16:22:32`，SHA256 `DD4A23DC6589ECE7648E907567B4711C749A9265334252F13B7C001E467A6320`；release zip 为 `16899588` bytes，时间 `2026-06-16 16:22:34`，SHA256 `69DD3721E843328308085F2EE16E799C49A414E149C47E0759EAB7584326829A`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实设置/网络小窗/标题栏图标视觉复验。

35. 心流 OpenAI Agents SDK 默认 runner 修复
   - 修正上一轮命名误导：`openai-agent` 不再是直接 chat completion 包装，而是 Ariadne 后端先本地检索/过滤 timeline、OCR、剪贴板、向量/关键词证据，再通过 `flow_agents_sdk_bridge.py` 启动 Python OpenAI Agents SDK sidecar。
   - SDK sidecar 使用 `openai-agents` 的 `Agent` / `Runner` / `OpenAIChatCompletionsModel`，读取 Ariadne AI provider/base/model 和安全 key；默认查找 `ARIADNE_FLOW_AGENT_PYTHON`、Ariadne app/local runtime 和系统 Python，默认跳过旧 x-tools `.venv`，只有 `ARIADNE_FLOW_AGENT_ALLOW_REPO_PYTHON=1` 才允许仓库 venv。
   - Codex 从默认心流问答路径里移出：只有 `settings.ai.codexCollaborationEnabled` 或 `ARIADNE_FLOW_AGENT=codex` 明确开启时才走 Codex runner；`ExternalAgentEnabled` 只保留外部代理任务包能力，不再让普通问答切到 Codex。
   - 针对“今天跟某某聊了什么”这类问题，意图识别增加 `跟/聊/说了什么`，并按问题中的人名、窗口标题、正文和标签重排联系人证据，避免只返回“今天沉淀了多少条”的静态统计。
   - Agent 未注册、SDK runtime 缺失、SDK 报错或返回空内容时，`AskFlow` 返回 `agent_error` 和明确错误提示，不再把本地摘要伪装为动态 AI 回复。
   - 心流/设置界面文案同步为 `OpenAI Agents SDK 问答` 和 `Agent sidecar`；直连 OpenAI-compatible runner 只作为内部兼容类型保留，mode 标记为 `agent:openai-compatible-direct`，不再伪装为 SDK。
   - 本机已创建 `%LOCALAPPDATA%\Ariadne\agent-python` 并安装 `openai-agents 0.17.5`；`from agents import Agent, OpenAIChatCompletionsModel, Runner` 导入验证通过；本地 mock server 无真实 key 烟测验证 SDK 调用链返回 `mock sdk answer`。
   - 验证命令：`go test ./internal/aiclient ./internal/workmemory -run "FlowAgent|AskFlow" -v`、清理 AI/Embedding 环境变量后的 `go test ./...`、`vue-tsc --noEmit`、`pnpm build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 199 Methods, 5 Enums, 183 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32405504` bytes，时间 `2026-06-16 17:04:58`，SHA256 `DA4CD5429493D16E12777EFF25F297957BBDCFFFB509F8B33F97E721A01AFD0A`；release zip 为 `16934844` bytes，时间 `2026-06-16 17:05:00`，SHA256 `901B60F908A3678856B07640881B35E33B60D477C230C2DC38A4B403F3916630`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面 Flow 提问复验。

36. 心流 Flow Memory Skill/CLI 化
   - 修正第 35 条仍由 Go 侧预先选择证据的问题：默认 `openai-agent` 路径不再依赖 Go 硬编码分词/意图命中来决定证据，而是把事实型问题交给通用 OpenAI Agents SDK runner。
   - 新增 `internal/workmemorycli`，`cmd/workmemoryctl` 和 `ariadne.exe workmemory ...` 共享同一个 JSON CLI。支持 `status`、`refresh`、`search`、`recent/timeline`、`get`、`add-note`；查询默认过滤敏感与 `qualityStatus=pending` 记录，并走外部 embedding/Milvus、SQLite FTS 和本地关键词 fallback。
   - `flow_agents_sdk_bridge.py` 新增 OpenAI Responses local shell Skill 分支：生成本地 `ariadne-flow-memory/SKILL.md`，用 SDK `ShellTool(environment.skills=[...])` 暴露给模型；本地 shell executor 仅允许读取该 `SKILL.md` 和运行 `ariadne workmemory ...`，拒绝其它命令。
   - 对当前 OpenAI-compatible Chat Completions provider（例如本机 `glm-5.1` 代理），明确降级为 OpenAI Agents SDK `function_tool` 调同一 CLI。该路径仍由 SDK `Agent`/`Runner` 驱动，但 UI 和返回 mode 标为 `agent:openai-agents-sdk-function-tool-fallback`，不再伪装成原生 shell Skill。
   - 前端心流回答 mode 标签补齐：原生路径显示 `OpenAI SDK Skill`，兼容降级显示 `OpenAI SDK 工具降级`，直连兼容 runner 继续显示 `OpenAI 兼容直连`。
   - 本轮工具路径台账：当前 Codex shell 的 PATH 不含 Go/Wails；实际可执行文件为 `C:\Users\luwei\.codex\tools\go1.26.4.windows-amd64\go\bin\go.exe`、`C:\Users\luwei\.codex\tools\go1.26.4.windows-amd64\go\bin\gofmt.exe`、`C:\Users\luwei\.codex\go-bin\wails3.exe`，后续应优先用这些绝对路径或先补 PATH。
   - 验证命令：bridge 解析自检、打包后 `ariadne.exe workmemory status` 返回 `work memory cli ready`、`go test ./internal/aiclient ./internal/workmemory ./internal/workmemorycli -v`、清理 AI/Embedding/OCR 环境变量后的 `go test ./...`、`vue-tsc --noEmit`、`pnpm build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `470 Packages, 23 Services, 200 Methods, 5 Enums, 184 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32547328` bytes，时间 `2026-06-16 18:16:03`，SHA256 `C2D682008089549828AB4B4C2DC5417D6D93208DB8B42537A92AB4BDA4BCD9FD`；release zip 为 `16997656` bytes，时间 `2026-06-16 18:16:04`，SHA256 `77ED5449F24E0897C4495D4BB648E3EAC2E21EA57E4E06CFA1AA2CBFBD7510D9`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面 Flow 提问复验。

37. 心流联系人证据边界与前台窗口采集默认
   - 针对用户反馈“我今天跟谁聊过天”人物关系不对，检查当前 `%APPDATA%\Ariadne\work_memory.json`、FTS 状态和 `capture_images` 原图后确认：OCR 基本读出了屏幕文字，但全屏微信截图同时包含当前会话、左侧会话列表、群预览、服务号和后台浏览器/Codex 内容，后续 OCR 整理和 Flow agent 把“界面可见人名”过度推断成“实际聊天对象”。这不是单纯 OCR 问题，而是采集证据污染加语义边界不够硬。
   - 收紧 `ocrSummaryPrompt`：聊天截图必须区分当前会话正文、左侧会话/联系人列表、后台窗口/侧边栏；右侧绿色气泡优先视为当前用户发送内容；服务号、群预览、侧栏名字和后台窗口名字不能直接作为聊天对象；不确定时写“界面可见”，不写“与某某沟通”。
   - 收紧 Flow agent system prompt、问答 prompt 和生成的 Flow Memory Skill：联系人类问题如“谁找过我/跟谁聊过/跟某某聊了什么”不能只凭 `search` 或 `recent` 摘要命名人物，必须把结果当候选并调用 `get` 查 OCR/详情；回答中要把确认聊天对象、群聊中出现的人名、侧栏/后台可见名字分开。
   - `workmemory` CLI 的 summary 输出新增微信类记录 `guidance`，提醒 agent 当前截图可能混有当前聊天、左侧列表、群预览、服务号和后台窗口；右侧绿色气泡大概率是当前用户，不能当成对方。
   - 自动时间机器默认采集范围从 `all_screens` 改为 `active_window`，settings schema 升至 v13；v13 迁移会把旧的空值或 `all_screens` 采集范围迁到 `active_window`，减少全屏干扰。打包后的 `ariadne.exe workmemory status` 已触发本机 `%APPDATA%\Ariadne\config.json` 读回迁移，当前配置为 `captureScope=active_window`。
   - 验证命令：`go test ./internal/settings ./internal/workmemory ./internal/workmemorycli ./internal/aiclient -v`、`go test ./...`、`vue-tsc --noEmit`、`pnpm build`、`wails3 generate bindings -clean=false`、打包后 `ariadne.exe workmemory status`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `470 Packages, 23 Services, 200 Methods, 5 Enums, 184 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32550912` bytes，时间 `2026-06-16 20:32:53`，SHA256 `B27AE147BB60B94FCAD2E5C5299C422667505676AEA3B9F39253BECD511256A0`；release zip 为 `16999305` bytes，时间 `2026-06-16 20:32:55`，SHA256 `7F62749556CB161E967B526EF75F6364B8D48CFE94686EFAE4988BACBDA511B3`。本轮只走后台命令行和本地截图文件检查，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面 Flow 提问复验。

38. 心流健康摘要、对话降噪与自主资产收件箱
   - 新增 `workmemory.Service.HealthSummary()`，导出 `HealthSummary` / `HealthAppStat` / `HealthRecentEvent`。它把现有的自动质检、待质检门禁、重复帧折叠、OCR 完成/待处理/失败、敏感记录隔离和按程序统计整理成 UI 可读状态，避免心流主界面继续靠原始时间线记录解释后台是否干净。
   - 心流首页改为对话优先：移除原来嵌在对话页里的自主产物卡片，只保留三张轻量健康卡片和少量后台事件。待质检记录会明确显示“未质检记录不会进入总结和沉淀”，OCR 和自动清理也只显示必要状态。
   - 资产页新增 `AUTONOMOUS INBOX`：心流每天或手动整理生成的 Skill、知识、日报、复盘统一进入收件箱；用户不删除即默认采纳。删除改为页内原因输入，原因会传给 `RejectAutonomousArtifact`，用于抑制未来同类产物。任务包、工作流、检查清单和 Skill 安装仍放在资产页下方，属于低频落地动作。
   - 前端 store 新增 `memory.health`，`load()`、自主整理和删除自主产物后都会刷新健康摘要；Wails bindings 已重新生成，新增方法/模型进入生产 bundle。
   - 验证命令：`go test ./internal/workmemory -run "HealthSummary|ReviewPendingCaptures|Autonomous|AskFlow" -v`、`vue-tsc --noEmit`、清理 AI/Embedding/OCR 环境变量后的 `go test ./...`、`pnpm build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `470 Packages, 23 Services, 201 Methods, 5 Enums, 187 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32593920` bytes，时间 `2026-06-16 21:13:58`，SHA256 `DCC313C37E03F2F53C2FEC6A74239F410FDF7E5F7AFDFA06C992B8327F16EFCF`；release zip 为 `17016930` bytes，时间 `2026-06-16 21:13:59`，SHA256 `FED2DE81B2A82DEC134E597C0E2F5037AD08AF85D9DEF76D9D3672B8D050EA71`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面视觉/点击复验。

39. 心流敏感判断凭据形态化
   - 针对用户反馈“现在对敏感的判断太敏感，正常截图也判断为敏感”，确认旧 `looksSensitive` 是纯关键词命中，`password`、`token`、`secret`、`验证码`、`密码` 等普通界面说明词会直接把截图/笔记/OCR 结果隔离。
   - `internal/workmemory` 新增共享的凭据形态正则，只在出现 key/value 密码、Bearer/Basic Authorization、Cookie/Set-Cookie、私钥块、SSH key、JWT/OpenAI/GitHub token 形态或中文 `密码/密钥/令牌/验证码: 值` 时自动敏感；普通 `密码输入框`、`token 配置说明`、`secret/API key 概念`、`验证码按钮` 不再自动置敏。
   - `internal/ocr` 删除重复敏感词判断，改为复用 `workmemory.LooksSensitiveText`；OCR 写回、质检 OCR、文本/文档/导出包导入、剪贴板沉淀都走同一条自动敏感策略。图片材料导入不再靠文件名/tag 推断敏感，`enrichEntry` 不再因为旧正文或标签关键词二次把正常记录改成敏感。
   - `main.applyWorkMemoryRuntime()` 现在把设置里的 `sensitiveRulesEnabled` 传入 `CapturePolicy`，运行时关闭自动敏感规则后不再自动隔离，但手动 `Sensitive=true` 仍然优先。
   - 新增/更新测试：`TestSensitiveDetectionRequiresCredentialShape`、`TestSensitiveRulesDoNotFlagPlainKeywordScreenshots`、`TestSensitiveRulesCanBeDisabledForAutomaticClassification`、`TestApplyOCRTextDoesNotMarkPlainSensitiveWords`、`TestRecognizeImageSensitiveFlagRequiresCredentialShape`。
   - 验证命令：`go test ./internal/workmemory ./internal/ocr -run "Sensitive|OCR|Quality|ReviewPending" -v`、清理 AI/Embedding/OCR 环境变量后的 `go test . ./cmd/... ./internal/...`、`pnpm --dir frontend build`、`wails3 generate bindings -clean=false`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 201 Methods, 5 Enums, 187 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32568320` bytes，时间 `2026-06-17 10:10:36`，SHA256 `62E35EC8E05750377B034987E3BB2B23B9AE3EDB0D2CB1E3BF094E38AB4740D4`；release zip 为 `17007560` bytes，时间 `2026-06-17 10:10:37`，SHA256 `92E0D1EA69427AAEAFEC6BC11F2A15C52F650C13720CF5A975D3532BC815426F`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面敏感截图 UI 复验。

40. 心流 Agent Responses 原生 Skill 兼容
   - 针对用户反馈“现在用的 AI 接口支持 responses 接口，可以改掉 openai-compatible 对 OpenAI Agent SDK 支持不够好的遗留问题”，确认旧 `flow_agents_sdk_bridge.py` 只有 OpenAI 官方/Azure 或显式 native env/payload 才走 `OpenAIResponsesModel + ShellTool`，`openai-compatible` 默认走 SDK `function_tool` 降级路径。
   - settings schema 升至 v14，新增 `agentResponsesEnabled` 并默认开启；迁移旧配置时自动补默认值，同时兼容 `agent_responses_enabled`、`agents_responses_enabled`、`agents_sdk_responses_enabled` 这几个 legacy preference key。
   - `workmemory.FlowAgentPolicy` / `FlowAgentJob` 新增 `NativeSkills`，`main.applyWorkMemoryAIRuntime()` 将设置开关注入 agent policy，`OpenAIAgentsSDKFlowAgent` 的 bridge payload 明确传递 `nativeSkills`。
   - Python bridge 在 `nativeSkills=true` 时让 `openai-compatible` 也优先尝试 OpenAI Agents SDK `OpenAIResponsesModel + ShellTool(environment.skills=[...])`；若兼容接口实际不支持 `/responses`，默认捕获错误并回退到原 `function_tool` 路径，只有 `ARIADNE_FLOW_AGENT_NATIVE_SKILLS_STRICT` 才直接返回 native 失败。
   - 设置中心和心流模型设置页新增 `Responses 原生 Skill` 开关，说明支持 `/responses` 时优先原生 ShellTool，失败回退工具降级；前端类型和 Wails bindings 已更新。
   - 新增/更新测试：`TestFlowAgentsSDKBridgePayloadEnablesNativeSkills`、`TestV13SettingsMigrateAgentResponsesEnabled`、`TestAskFlowUsesFlowAgentRunnerWhenConfigured` 的 native skill policy 断言，以及默认设置启用断言。
   - 验证命令：`go test ./internal/settings ./internal/aiclient ./internal/workmemory -run "AgentResponses|FlowAgent|AskFlow|Settings|Sensitive" -v`、清理 AI/Embedding/OCR 环境变量后的 `go test . ./cmd/... ./internal/...`、`wails3 generate bindings -clean=false`、`pnpm --dir frontend build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 201 Methods, 5 Enums, 187 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32570880` bytes，时间 `2026-06-17 10:18:00`，SHA256 `A2DFF4DF49A6F98422FA7B007C45F438C6D57958582397BE58B8291E9E15DD4C`；release zip 为 `17008416` bytes，时间 `2026-06-17 10:18:01`，SHA256 `2D2D40B0D0BBE92D0D3E226EBBDB812190BC9E0B4164E9E5C3BE5DA7B3ACE7F3`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面 Flow Agent 提问复验。

41. 心流 Agent tool_call 泄漏防护
   - 针对用户截图中 OpenAI SDK Skill 回答直接显示 `<tool_call>shell<arg_key>command</arg_key>...SKILL.md...</tool_call>`，确认这是 Responses-compatible 接口把模型生成的工具调用标记当普通文本返回，Agents SDK 没有真正执行 ShellTool，而 Ariadne 旧逻辑把 `final_output` 直接当最终回答。
   - `flow_agents_sdk_bridge.py` 新增 `_looks_like_unexecuted_tool_call()`，识别 `<tool_call>`、`<arg_key>/<arg_value>` 和 JSON `tool_calls` / `tool_call` 形态。
   - 原生 Responses ShellTool 路径如果返回未执行 tool-call 文本，不再 `ok=true`，而是返回 native failure 触发已有 function-tool fallback；function-tool fallback 如果仍返回同类协议文本，则直接失败，避免把内部工具协议展示给用户。
   - Go 侧 `OpenAIAgentsSDKFlowAgent` 和 `OpenAICompatibleFlowAgent` 增加同样的兜底拒绝，防止任意 bridge 或直连 runner 把未执行工具调用作为正常回答进入 UI。
   - 新增 `TestLooksLikeUnexecutedAgentToolCall`，覆盖截图中的 XML shell call、JSON tool_calls 和正常中文回答。
   - 验证命令：`python -m py_compile internal\aiclient\flow_agents_sdk_bridge.py`、bridge helper smoke、`go test ./internal/aiclient -run "Unexecuted|FlowAgent" -v`、清理 AI/Embedding/OCR 环境变量后的 `go test . ./cmd/... ./internal/...`、打包后 `ariadne.exe workmemory status`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 201 Methods, 5 Enums, 187 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32573440` bytes，时间 `2026-06-17 10:35:20`，SHA256 `C08D8C0EFE34B60814915F81EA6282D917DCC43E3E8794D936753B82EA3DE047`；release zip 为 `17011440` bytes，时间 `2026-06-17 10:35:21`，SHA256 `AF8D40F7ED4950A41799D6CB8CEB5A8364C252C72A863D8A82616F65430B18E2`。本轮只走后台命令行，未操作鼠标、窗口、热键或桌面画面，因此不声明真实桌面 Flow Agent 提问复验。

42. 心流首页对话区压缩与 Markdown 渲染
   - 针对用户反馈“上半部分占据了太大空间，统计卡片没必要放首页，最上面信息可以去掉，对话框要支持 markdown 渲染”，确认首页首屏同时存在外层状态/动作栏、页面标题/状态、健康统计卡和健康事件 chip，导致长回答区被明显挤压。后续用户截图指出欢迎气泡贴顶仍然很丑，并要求使用 Product Design 重新设计；Product Design context preflight 未发现保存的额外上下文，因此以当前 Ariadne 截图和现有设计系统为视觉来源。
   - `activeFlowPage === 'flow'` 时隐藏外层 `flow-stage-top`，非首页仍保留自主整理、主动沉淀、采集和隐私模式按钮。
   - 删除首页内 `flow-chat-header`、`flow-health-strip` 和 `flow-health-events` 渲染，移除对应首页专用 computed；自主产物摘要仍保留在资产页。
   - 首页布局改为 `.flow-stage.is-chat-home` 单行内容区，`.flow-chat-home` 高度占满并隐藏外层滚动，`.flow-chat-thread` 吃满剩余空间，composer 留在底部；空会话时新增 `.flow-chat-thread.is-empty`，将唯一欢迎消息作为中段 centered starter card，而用户开始提问后恢复正常从上到下的消息流。
   - 欢迎消息增加 `.flow-message.is-system` 视觉状态，使用轻量 starter card、居中构图、较柔和的头像和阴影，避免删除 header 后第一条消息贴在窗口顶边。
   - 消息气泡从纯文本 `<p>` 改为 `flowMessageHtml()` 的安全 Markdown 渲染：先转义原始 HTML，再支持标题、无序/有序列表、引用、代码块、行内代码、HTTP(S) 链接和 `**加粗**`；新增 `.flow-message-markdown` 排版样式，让长回答中的 `##`、`**`、列表不再原样显示。
   - 验证命令：Product Design context preflight、`pnpm --dir frontend build`、`wails3 task windows:package` 均通过；Vite/Rolldown 仍有第三方 `@vueuse/core` pure annotation 警告和大 chunk 提示，不影响产物。最新 bindings 为 `469 Packages, 23 Services, 201 Methods, 5 Enums, 187 Models, 0 Events`；最新 `bin\ariadne.exe` 为 `32574464` bytes，时间 `2026-06-17 11:29:24`，SHA256 `204E3396FF3E5942EF541704BA237159D9A77A4679198571FAF19F7E6BCA0D83`；release zip 为 `17011986` bytes，时间 `2026-06-17 11:29:25`，SHA256 `D363FF4EAA374F45E4A98A6583722CAAE9B308492FF9CE3D43F9F009A73E23EF`。本轮只走后台命令行，未操作鼠标、窗口、热键、浏览器或桌面画面，因此不声明真实桌面视觉复验。
...
4. 工作记忆
   ...
   - 本地 OCR 写回、时间机器自动 OCR 写回、SQLite FTS、本地经验发现首版、经验发现处理状态、本地日报草稿、心流 AskFlow 问答契约、OpenAI Agents SDK sidecar 动态回答、可选 Codex runner、心流首页对话框化、对话消息右键/多选显式加入沉淀、AI 日报润色首版、外部 AI 经验发现、外部 embedding、内置向量缓存和 Milvus REST 向量存储首版、支持多选证据组的独立问题复盘草稿、本地定期草稿调度、候选工作流草稿、检查清单草稿、候选工作流正式保存、候选检查清单正式保存、本地 Skill 正式保存、Codex skill 包导出、live 安装和 Ariadne refresh marker 握手已接入；旧版工作记忆 `entries.json` 历史迁移首版已接入；Markdown/文本、图片、PDF、Office 文档和 Ariadne 导出 zip 材料导入、筛选导出、五类排除规则配置和时间机器回放代码路径已接入。运行中 Codex 实际热加载 newly installed skill 仍未验收；外部 AI 经验发现仍缺真实桌面二次确认流程验收；心流问答和对话沉淀已做后端、SDK bridge、Codex 可选 runner 和前端构建验收，仍缺真实桌面提问/右键/多选/加入沉淀点击复验。
...
5. 设置
   ...
   - API key 等敏感凭据已接入 Windows Credential Manager 首版；AI/embedding 当前密钥已写入本机 Credential Manager，运行时环境变量仍优先。设置中心密钥保存/清除按钮、心流设置“模型与向量”页签密钥保存/清除入口和旧配置密钥迁移 notes 已补 Go/前端构建级验证；剩余差距是真实桌面点击烟测，以及发布安装后 Credential Manager 读写权限复验。
