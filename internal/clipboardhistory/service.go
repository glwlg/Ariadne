...
func (s *Service) ImportLegacyEntries(entries []Entry) Status {
	...
	for _, entry := range imported {
		s.addEntryLockedWithSave(entry, false)
	}
	if len(imported) > 0 {
...
func (s *Service) addEntryLocked(entry Entry) Entry {
	return s.addEntryLockedWithSave(entry, true)
}

func (s *Service) addEntryLockedWithSave(entry Entry, immediateSave bool) Entry {
	entry = normalizeEntry(entry)
	if !entryIsValid(entry) {
		s.removeEntryFile(entry)
		return Entry{}
...
func (s *Service) runWatcher(stop <-chan struct{}) {
	...
	err := watchSystemClipboard(stop, func() {
		select {
		case events <- struct{}{}:
		default:
		}
...
func (s *Service) pollClipboardOnce(recordChange bool) Status {
	...
	s.lastWatcherSignature = signature
	entry = s.addEntryLockedWithSave(entry, false)
	status := s.statusLocked(true)
	s.mu.Unlock()
	s.notifyEntryObservers(entry)
	return status
