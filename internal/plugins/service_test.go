...
func TestLegacyPythonBuiltinsHaveNativeGoCoverage(t *testing.T) {
	...
	legacyPluginDir := filepath.Join(repoRoot, "legacy", "x-tools-python", "src", "plugins")
	legacyFiles, err := filepath.Glob(filepath.Join(legacyPluginDir, "*.py"))
	if err != nil {
		t.Fatalf("glob legacy plugins: %v", err)
	}
	if len(legacyFiles) == 0 {
		t.Fatalf("expected legacy Python plugins under %s", legacyPluginDir)
	}
